...
const NavigationBar = ({ cartCounter, updateCounter, setCartCounter }) => {
  ...
  const navObserver = new IntersectionObserver((entries) => {
      navigationRef.current.classList.toggle("sticky", !entries[0].isIntersecting)

      const mediaQuery = window.matchMedia("(max-width: 550px)")

      if (mediaQuery.matches) {
        navigationRef.current.classList.toggle("slimmer-nav", !entries[0].isIntersecting)

      } else {
        navigationRef.current.classList.remove("slimmer-nav")
 ...
 useEffect(() => {
  ...
  return (
    <>
      <div data-scroll-watcher="" ref={scrollWatcher}></div>
      <section className="navigation" ref={navigationRef}>
